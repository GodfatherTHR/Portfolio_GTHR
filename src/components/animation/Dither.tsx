...
vec3 dither(vec2 uv, vec3 color, vec2 resolution, float pixelSize) {
...
void mainImage(in vec4 inputColor, in vec2 uv, out vec4 outputColor) {
  vec2 resolution = vec2(1.0); // This will be set by the composer
  vec2 normalizedPixelSize = pixelSize / resolution;
  vec2 uvPixel = normalizedPixelSize * floor(uv / normalizedPixelSize);
  vec4 color = texture2D(inputBuffer, uvPixel);
  color.rgb = dither(uv, color.rgb, resolution, pixelSize);
  outputColor = color;
}
`;

class RetroEffectImpl extends Effect {
  constructor(props = { colorNum: 4.0, pixelSize: 2.0 }) {
    const uniforms = new Map<string, THREE.Uniform<any>>([
      ["colorNum", new THREE.Uniform(props.colorNum)],
      ["pixelSize", new THREE.Uniform(props.pixelSize)],
    ]);
